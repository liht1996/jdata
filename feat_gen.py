...
def feat_extract(end_date,time_gap,order,action,user_info):
    ...
    for gap in order_tmp_dict.keys():
        ...
        user_fea['10130_price_sum'] = user_fea['10130_price_sum'].map(log1p)
        #近期平均商品价格 s
        ...
        user_fea['10130_price_mean'] = user_fea['10130_price_mean'].map(log1p)
        #近期最小商品价格 s
        ...
        user_fea['10130_price_min'] = user_fea['10130_price_min'].map(log1p)
        #近期最大商品价格 s
        ...
        user_fea['10130_price_max'] = user_fea['10130_price_max'].map(log1p)
        #近期订单金额中位数 s
        ...
        user_fea['order_num'] = user_fea['order_num'].map(log1p) 
        ...
        user_fea['10130_order_num'] = user_fea['10130_order_num'].map(log1p) 

       
        #近期购买商品的次数 s
        user_fea['sku_count'] = order_tmp.pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'count'})
#         user_fea['sku_count'] = user_fea['sku_count'].map(log1p)
...
#         user_fea['10130_sku_count'] = user_fea['10130_sku_count'].map(log1p)
#         
        
        
        #近期的商品购买数 s
        user_fea['sku_num'] = order_tmp.pivot_table(index='user_id',values='o_sku_num',aggfunc={'o_sku_num':'sum'})
#         user_fea['sku_num'] = user_fea['sku_num'].map(log1p)
...
#         user_fea['10130_sku_num'] = user_fea['10130_sku_num'].map(log1p)
        ...
        # user_fea['sku_unique'] = user_fea['sku_unique'].map(log1p)
        ...
        # user_fea['10130_sku_unique'] = user_fea['10130_sku_unique'].map(log1p)

        
        #近期购买的sku商品次数/SKU商品种类，这个指标可以反映用户对商品的购买集中度 （新加）
#         user_fea['sku_focus']= user_fea['sku_count']/user_fea['sku_unique']
        user_fea['10130_sku_focus']= user_fea['10130_sku_count']/user_fea['10130_sku_unique']        
#         user_fea.ix[user_fea['sku_focus']==float('inf'),'sku_focus']=0#正无穷置为0
...
#         tmp = order_tmp.groupby(['user_id','sku_id'],as_index=False).agg({'o_id':'count'})#这个as_index要注意记下来
#         user_fea['samesku_maxcount']= tmp.pivot_table(index='user_id',values='o_id',aggfunc={'o_id':'max'})        
        ...
        user_fea['10130_samesku_maxcount'] = user_fea['10130_samesku_maxcount'].map(log1p)       

        #用户101.30历经的下单地点数量(3609基础上新加)
        ...
        #用户101.30历经的下单最多的oarea(3609基础上新加)
...
#         o_area_mode.reset_index('o_area')
        ...
        #如果时间窗口是3个月或者6个月，计算月首购买day的最大，最小，平均，中位数（新加）
        ...
        # user_fea['order_day_num'] = user_fea['order_day_num'].map(log1p)
        ...
        # user_fea['10130_order_day_num'] = user_fea['10130_order_day_num'].map(log1p)
        ...
        user_fea['10130_act_sku_sum'] = user_fea['10130_act_sku_sum'].map(log1p)

        #近期的商品收藏的数量（新加）s
        user_fea['col_sku_sum'] = action_tmp.ix[action_tmp['a_type']==2,:].pivot_table(index='user_id',values='a_num',aggfunc={'a_num':'sum'})
        user_fea['col_sku_sum'] = user_fea['col_sku_sum'].map(log1p)
        ...
        user_fea['10130_col_sku_sum'] = user_fea['10130_col_sku_sum'].map(log1p)
       
#         #近期的商品（收藏或浏览）行为count：我觉得这个特征十分的不合理
#         user_fea['act_sku_count'] = action_tmp.pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'count'})
#         user_fea['10130_act_sku_count'] = action_tmp.ix[action_tmp['cate'].isin([101,30]),:].pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'count'})
# #         user_fea['30_act_sku_count'] = action_tmp.ix[action_tmp['cate'].isin([30]),:].pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'count'})
# #         user_fea['101_act_sku_count'] = action_tmp.ix[action_tmp['cate'].isin([101]),:].pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'count'})
#          
#          
#         #近期的商品浏览count：我觉得这个特征十分的不合理
#         user_fea['see_sku_count'] = action_tmp.ix[action_tmp['a_type']==1,:].pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'count'})
#         user_fea['10130_see_sku_count'] = action_tmp.ix[(action_tmp['cate'].isin([101,30]))&(action_tmp['a_type']==1),:].pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'count'})
# #         user_fea['30_see_sku_count'] = action_tmp.ix[(action_tmp['cate'].isin([30]))&(action_tmp['a_type']==1),:].pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'count'})
# #         user_fea['101_see_sku_count'] = action_tmp.ix[(action_tmp['cate'].isin([101]))&(action_tmp['a_type']==1),:].pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'count'})
# #         user_fea['other_see_num'] = action_tmp.ix[(action_tmp['cate'].isin([1,46,71,83]))&(action_tmp['a_type']==1),:].pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'count'})       
#  
#         #近期的商品收藏count：我觉得这个特征十分的不合理
#         user_fea['col_sku_count'] = action_tmp.ix[action_tmp['a_type']==2,:].pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'count'})
#         user_fea['10130_col_sku_count'] = action_tmp.ix[(action_tmp['cate'].isin([101,30]))&(action_tmp['a_type']==2),:].pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'count'})
# #         user_fea['30_col_sku_count'] = action_tmp.ix[(action_tmp['cate'].isin([30]))&(action_tmp['a_type']==2),:].pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'count'})
# #         user_fea['101_col_sku_count'] = action_tmp.ix[(action_tmp['cate'].isin([101]))&(action_tmp['a_type']==2),:].pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'count'})
# #         user_fea['other_see_num'] = action_tmp.ix[(action_tmp['cate'].isin([1,46,71,83]))&(action_tmp['a_type']==1),:].pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'count'})       
        
        
        #近期有（浏览或收藏）行为的天数
        user_fea['act_day_num'] = action_tmp.pivot_table(index='user_id',values='a_date',aggfunc={'a_date':'nunique'})
        # user_fea['act_day_num'] = user_fea['act_day_num'].map(log1p)
        ...
        # user_fea['10130_col_day_num'] = user_fea['10130_col_day_num'].map(log1p)


        #近期有（浏览或收藏）行为的商品数（种类）
        user_fea['act_unique'] = action_tmp.pivot_table(index='user_id',values='sku_id',aggfunc={'sku_id':'nunique'})
        # user_fea['act_unique'] = user_fea['act_unique'].map(log1p)
        ...
        # user_fea['10130_col_unique'] = user_fea['10130_col_unique'].map(log1p)
        ...
        # user_fea['com_score1_count'] = user_fea['com_score1_count'].map(log1p)
#         user_fea['101_com_score1_count']=order_tmp_com.ix[order_tmp_com['cate'].isin([101]),:].pivot_table(index='user_id',values = 'score_level_1_count',aggfunc={'score_level_1_count':'sum'})
#         user_fea['30_com_score1_count']=order_tmp_com.ix[order_tmp_com['cate'].isin([30]),:].pivot_table(index='user_id',values = 'score_level_1_count',aggfunc={'score_level_1_count':'sum'})
        #中评数量
        ...
        # user_fea['com_score2_count'] = user_fea['com_score2_count'].map(log1p)
#         user_fea['101_com_score2_count']=order_tmp_com.ix[order_tmp_com['cate'].isin([101]),:].pivot_table(index='user_id',values = 'score_level_2_count',aggfunc={'score_level_2_count':'sum'})
#         user_fea['30_com_score2_count']=order_tmp_com.ix[order_tmp_com['cate'].isin([30]),:].pivot_table(index='user_id',values = 'score_level_2_count',aggfunc={'score_level_2_count':'sum'})
        #差评数量
        ...
        # user_fea['com_score3_count'] = user_fea['com_score3_count'].map(log1p)
#         user_fea['101_com_score3_count']=order_tmp_com.ix[order_tmp_com['cate'].isin([101]),:].pivot_table(index='user_id',values = 'score_level_3_count',aggfunc={'score_level_3_count':'sum'})
#         user_fea['30_com_score3_count']=order_tmp_com.ix[order_tmp_com['cate'].isin([30]),:].pivot_table(index='user_id',values = 'score_level_3_count',aggfunc={'score_level_3_count':'sum'})
     
        
        
        #用户近期（101,30）好评，中评，差评的数量（新加入）
        ...
        # user_fea['10130_com_score1_count'] = user_fea['10130_com_score1_count'].map(log1p)
#         user_fea['101_com_score1_count']=order_tmp_com.ix[order_tmp_com['cate'].isin([101]),:].pivot_table(index='user_id',values = 'score_level_1_count',aggfunc={'score_level_1_count':'sum'})
#         user_fea['30_com_score1_count']=order_tmp_com.ix[order_tmp_com['cate'].isin([30]),:].pivot_table(index='user_id',values = 'score_level_1_count',aggfunc={'score_level_1_count':'sum'})
        #中评数量
        ...
        # user_fea['10130_com_score2_count'] = user_fea['10130_com_score2_count'].map(log1p)
#         user_fea['101_com_score2_count']=order_tmp_com.ix[order_tmp_com['cate'].isin([101]),:].pivot_table(index='user_id',values = 'score_level_2_count',aggfunc={'score_level_2_count':'sum'})
#         user_fea['30_com_score2_count']=order_tmp_com.ix[order_tmp_com['cate'].isin([30]),:].pivot_table(index='user_id',values = 'score_level_2_count',aggfunc={'score_level_2_count':'sum'})
        #差评数量
        ...
        # user_fea['10130_com_score3_count'] = user_fea['10130_com_score3_count'].map(log1p)
#         user_fea['101_com_score3_count']=order_tmp_com.ix[order_tmp_com['cate'].isin([101]),:].pivot_table(index='user_id',values = 'score_level_3_count',aggfunc={'score_level_3_count':'sum'})
#         user_fea['30_com_score3_count']=order_tmp_com.ix[order_tmp_com['cate'].isin([30]),:].pivot_table(index='user_id',values = 'score_level_3_count',aggfunc={'score_level_3_count':'sum'})
